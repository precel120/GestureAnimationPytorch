...
print(f"Model loaded: {time.time() - start}")
start = time.time()
# train_g_losses, train_d_losses = train.train_pix2pix(display_interval)

train.generate_image('./result.jpg', './generated_single.jpg')
print(f'Generated image in {time.time() - start}')
start = time.time()
...
print(f'Generated sequence in {time.time() - start}')
start = time.time()

create_animation('./frames', './animation.mp4')
print(f'Created animation in {time.time() - start}')
